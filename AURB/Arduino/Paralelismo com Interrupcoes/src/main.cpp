...
void speedISR(){
  if (SPEED == LOW_SPEED) {
    ...
    Serial.println("Velocidade rápida");
  }
...
{ 
  ...
  attachInterrupt(digitalPinToInterrupt(BTN_PAUSE), pauseISR, RISING);
  pinMode (BTN_SPEED, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(BTN_SPEED), speedISR, RISING);
